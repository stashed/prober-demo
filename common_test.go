...
func (p fakeExecProber) Probe(e exec.Cmd) (probe.Result, string, error) {
	var res interface{}
	res = p.result
	return res.(probe.Result), "", p.err
}
...
func (p *syncExecProber) Probe(cmd exec.Cmd) (probe.Result, string, error) {
	...
	defer p.RUnlock()
	var res interface{}
	res, str, err := p.fakeExecProber.Probe(cmd)
	return res.(probe.Result), str, err
}
