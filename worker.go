...
type worker struct {
	...
	initialValue kresults.Result

	// Where to store this workers kresults.
	resultsManager kresults.Manager
	probeManager   *manager

	// The last known container ID for this worker.
	containerID kubecontainer.ContainerID
	// The last probe result for this worker.
	lastResult kresults.Result
...
func newWorker(
	...
	case readiness:
		...
		w.initialValue = kresults.Failure
	case liveness:
		w.spec = container.LivenessProbe
		w.resultsManager = m.livenessManager
		w.initialValue = kresults.Success
	}
...
func (w *worker) doProbe() (keepGoing bool) {
	...
	if c.State.Running == nil {
		klog.V(3).Infof("Non-running container probed: %v - %v",
			format.Pod(w.pod), w.container.Name)
		if !w.containerID.IsEmpty() {
			w.resultsManager.Set(w.containerID, kresults.Failure, w.pod)
		}
	...
	if (result == kresults.Failure && w.resultRun < int(w.spec.FailureThreshold)) ||
		(result == kresults.Success && w.resultRun < int(w.spec.SuccessThreshold)) {
	...
	if w.probeType == liveness && result == kresults.Failure {
